...
ggplot(data = df, aes(x = date, y = discount)) + 
  ...
  labs(x = "Date",
       y = "Discount",
       title = "Holding's discount in relation to shares held 
...
Dashed red vertical line represents the mean. Distribution is rigth-skewed, which means that in some occasions the discount was too high (more than 30%). Numerically, let's view the summary stats:
...
There are many ways of visualizing if there are difference on discounts in both situations. Through a boxplot:
...
data %>%
  ...
  geom_text(aes(label = percent(prob)), hjust = -.1, vjust = -1) + 
  xlim(c(.5,1))
```

As expected, the higher the discount, the higher the probability of ITSA4 returns exceed ITUB4 returns. But, again, paradoxically, there is a drop on success probability in the maximum range of discount (24.8% to 30.4%). We can suspect that success depends on other factors than discount - for instance, when the observations were obtained (or, in other words, the passage of time). We can check this coloring time series' discount, by success or failure:
...
At Jun 2021, ITSA4 was being traded at a discount of 24%. And then, are you going do buy, keep or sell?
